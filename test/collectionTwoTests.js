...
describe("ERC721 PongHeroPets V1", async function () {
  let collectionOne;
  let coll;
  
  beforeEach(async () => {
    collectionOne = await ethers.getContractFactory("PongHeroPetsV1");
    //coll = await upgrades.deployProxy(collectionOne,["PongHeroPets","PHP","https://example.com/"],{initializer: 'initialize'});
    coll = await collectionOne.deploy("PongHeroPets","PHP","https://example.com/");
    await coll.deployed();

  });

  it("Checking Name", async function () {
    expect(await coll.name()).to.equal("PongHeroPets");
  });

  it("Checking Symbol", async function () {
    expect(await coll.symbol()).to.equal("PHP");
  });
  ...
  it("Checking Max Tokens Supply", async function () {
    expect(await coll.maxSupply()).to.equal(8000);
  });
